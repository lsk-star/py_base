...
def generate_module(project_root: Path, options: ModuleOptions) -> GenerationResult:
    ...
    module_directory = project_root / "src" / "pybase" / "modules" / module_name
...
def _validate_options(project_root: Path, options: ModuleOptions) -> None:
    ...
    required_paths = [
        ...
        project_root / "src" / "pybase" / "modules" / "registry.py",
    ]
    if options.with_model:
    ...
    if missing_paths:
        raise GenerationError("当前目录不是完整的 PyBase 项目: " + ", ".join(missing_paths))
...
4. 在 `service.py` 编排 Repository，并在这里划定事务边界。
...
from pybase.modules.$module_name.router import router
...
class ${module_class_name}Service:
    ...
    # 在此添加业务方法。事务边界应由 Service 管理，日志使用本模块 logger。
""",
        values,
    )
...
class ${module_class_name}Service:
    \"\"\"$module_class_name 模块的业务用例和事务边界。\"\"\"
    ...
    async def delete_by_id(self, entity_id: int) -> bool:
        ...
        async with self._session.begin():
            return await self._repository.delete_by_id(entity_id)
...
def _build_database_files(module_directory: Path, values: dict[str, str]) -> dict[Path, str]:
    return {
        module_directory / "models.py": _render(
            """from pybase.infrastructure.database.base import Base, IntIdMixin
...
from pybase.common.pagination import PageParams, PageResult
...
def _upgrade_service(
    *,
    ...
    values = {"module_name": module_name, "module_class_name": module_class_name}
...
def _upsert_model_import(
    *,
    ...
    return _upsert_between_markers(
        ...
        entry=f"from pybase.modules.{module_name}.models import {module_class_name}",
        entry_pattern=(
            rf"^[ \t]*from pybase\.modules\.{re.escape(module_name)}\.models import [^\r\n]+$"
        ),
        path=path,
    )
...
def _registry_entry(module_name: str, with_model: bool) -> str:
    return (
        ...
        f'router_path="pybase.modules.{module_name}.router:router", '
